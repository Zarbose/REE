...
#création des répertoires
  ...
  mkdir -p /srv/empreinte/keyPubRasp
...
keyPubRasp="/srv/empreinte/keyPubRasp/key.pub"
keyPubRaspTraiter="/srv/empreinte/keyPubRasp/keyTraiter.pub"
...
openssl pkey -in connexion/DH/prive/private_key.pem -pubout -out connexion/DH/publique/public_key.pem
...
while [ "$result" != "ACK" ]; do
    ...
    recv $CLIENT || logger -e "Erreur de reception !" >&2

    msg="AckSyn"
    send $SERVEUR "$msg" || logger -e "Erreur de transmission (AckSyn)!" >&2

    logger -e "le client a envoyé le ACK et attend un message" >&2
...
res=""
FIN="FIN"
echo "-----BEGIN PUBLIC KEY-----" > connexion/DH/keyServeur/public_key.pem
...
while true; do
    ...
    echo "$res" >> connexion/DH/keyServeur/public_key.pem
done

echo "-----END PUBLIC KEY-----" >> connexion/DH/keyServeur/public_key.pem
...
openssl pkeyutl -derive -inkey connexion/DH/prive/private_key.pem -peerkey connexion/DH/keyServeur/public_key.pem -out connexion/DH/secret/shared_key.bin
...
hexdump -e '32/1 "%02x"' connexion/DH/secret/shared_key.bin | cut -c 1-25 > configWPA/mdp/mdp.txt
...
echo "" |sudo tee /home/rasp/.ssh/authorized_keys

logger -e "Le fichier authorized_keya été vidé !"

cat $keyPubRaspTraiter |sudo tee /home/rasp/.ssh/authorized_keys
