...
#on attends 10s le cleint
logger "On attends le clt 1 munite !!"
sleep 1m

#création des répertoire nécessaire
...
#logger -e "********************** Envoi de la clé publique du serveur au client: $CLIENT *********************************"
sleep 5
# Variable de contrôle pour suivre l'état de la transmission
transmission_ok=true

while IFS= read -r line; do
    send "$CLIENT" "$line" || {
        logger -e "Erreur de transmission dans l'envoi de la clé publique du serveur au client: $CLIENT!" >&2
        transmission_ok=false
        break
    }
    logger -e "Ligne clé publique = $line"
    sleep 3
done < <(head -n -1 "$keyPub" | tail -n +2)

# Vérifier si la transmission a réussi avant de continuer
if [ "$transmission_ok" = true ]; then
    # Transmission réussie, envoyer la marque de fin (FIN)
    send "$CLIENT" "FIN" || logger -e "Erreur de transmission de (FIN) !" >&2
logger -e "La clé publique du serveur a été envoyé avec succès!"
else
    logger -e "La transmission a échoué, script arrêté."
fi
...
openssl pkeyutl -derive -inkey $keyPrive -peerkey $keyClient -out $secret
